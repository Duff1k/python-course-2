...
keyword = ['Дата и время', 'Вид спорта', 'Тренер', 'Зал']
TRAININGS = []
...
for i in range(1, 5):
    ...
    sorted_TRAININGS = sorted(TRAININGS, key=lambda x: x['Дата и время'], reverse=False)
    TRAININGS.clear()
    ...
    for tr in sorted_TRAININGS:
        ws.append([tr["Тренер"], tr["Вид спорта"], tr["Дата и время"]])
